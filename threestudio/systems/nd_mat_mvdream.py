...
@threestudio.register("nd-mat-mvdream-system")
class NDMVDreamSystem(BaseLift3DSystem):
    ...
    def configure(self) -> None:
        ...
        self.has_mv_guidanece = (self.cfg.mv_guidance_type != "none") and hasattr(
            self.cfg.loss, "lambda_mv"
        )  # and (self.cfg.loss.lambda_rgb_sd > 0)
        threestudio.info(
            f"================has_nd_guidanece:{self.has_nd_guidanece}, has_rgb_sd_guidanece:{self.has_rgb_sd_guidanece}, has_mv_guidanece:{self.has_mv_guidanece}================="
        )
        ...
        if self.has_mv_guidanece:
            self.mv_guidance = threestudio.find(self.cfg.mv_guidance_type)(
                self.cfg.mv_guidance
            )
            self.mv_guidance.requires_grad_(False)
            self.mv_prompt_processor = threestudio.find(
                self.cfg.mv_prompt_processor_type
            )(self.cfg.mv_prompt_processor)
            self.mv_prompt_utils = self.mv_prompt_processor()
    ...
    def on_load_checkpoint(self, checkpoint):
        ...
        if self.has_mv_guidanece:
            guidance_mv_state_dict = {
                "mv_guidance." + k: v
                for (k, v) in self.mv_guidance.state_dict().items()
            }
            checkpoint["state_dict"] = {
                **checkpoint["state_dict"],
                **guidance_mv_state_dict,
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.has_mv_guidanece = (
            (self.cfg.mv_guidance_type != "none")
            and hasattr(self.cfg.loss, "lambda_mv")
            and (self.C(self.cfg.loss.lambda_mv) > 0)
        )
        print(
            f"has_nd_guidanece:{self.has_nd_guidanece}, has_rgb_sd_guidanece:{self.has_rgb_sd_guidanece}, has_mv_guidanece:{self.has_mv_guidanece}"
        )
        ...
        if not self.cfg.texture:  # geometry training
            ...
            if self.has_mv_guidanece:
                if self.true_global_step < self.cfg.mv_latent_steps:
                    mv_guidance_inp = self.collect_inputs(
                        out, self.cfg.mv_guidance.collect_inputs_lat
                    )
                    mv_guidance_inp = mv_guidance_inp * 2.0 - 1.0

                    mv_guidance_out = self.mv_guidance(
                        mv_guidance_inp,
                        self.mv_prompt_utils,
                        **batch,
                        rgb_as_latents=True,
                    )
                else:
                    mv_guidance_inp = self.collect_inputs(
                        out, self.cfg.mv_guidance.collect_inputs
                    )
            ...
            if self.has_rgb_sd_guidanece:
                ...
                if self.true_global_step < self.cfg.latent_steps:
                    ...
                    if self.cfg.switch_ginp:
                        ...
                        print("switch: ", self.true_global_step, collect_inps)
            ...
            if self.has_mv_guidanece:
                ...
                mv_guidance_out = self.mv_guidance(
                    mv_guidance_inp, self.mv_prompt_utils, **batch, rgb_as_latents=False
                )
        ...
        if self.has_mv_guidanece:
            for name, value in mv_guidance_out.items():
                if name != "timestep":
                    self.log(f"train/mv_{name}", value)
                if name.startswith("loss_"):
                    loss_mv += value * self.C(
                        self.cfg.loss[name.replace("loss_", "lambda_mv_")]
                    )
            mv_wight = (
                self.C(self.cfg.loss.lambda_mv_w)
                if hasattr(self.cfg.loss, "lambda_mv_w")
                else 1.0
            )
            loss += loss_mv * mv_wight  # * self.C(self.cfg.loss.lambda_nd)

        print(f"loss_rgb_sd:{loss_rgb_sd}, loss_nd:{loss_nd}, loss_mv:{loss_mv}")
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.save_image_grid(
            ...
            #         {
            #             "type": "rgb",
            #             "img": out["comp_diffuse_albedo_light"][0],
            #             "kwargs": {"data_format": "HWC", "data_range": (0, 1)},
            #         }
            #     ]
            #     if "comp_diffuse_albedo_light" in out
            #     else []
            # )
            # + (
            #     [
            #         {
            #             "type": "rgb",
            #             "img": out["comp_specular_albedo_light"][0],
            #             "kwargs": {"data_format": "HWC", "data_range": (0, 1)},
            #         }
            #     ]
            #     if "comp_specular_albedo_light" in out
            #     else []
            # )
            + (
                [

...
class NeRFVolumeRenderer(VolumeRenderer):
    ...
    def forward(
        ...
        **kwargs,
        ...
        mat_all = {}
        if isinstance(rgb_fg_all, dict):
            assert "rgb" in rgb_fg_all
            mat_all = rgb_fg_all
        else:
            mat_all["rgb"] = rgb_fg_all
        out_sub = {}
        for k, v in mat_all.items():
            comp_value_fg: Float[Tensor, "Nr Nc"] = nerfacc.accumulate_along_rays(
                weights[..., 0], values=v, ray_indices=ray_indices, n_rays=n_rays
            )
            if k == "rgb" or k == "albedo":
                comp_value = comp_value_fg + bg_color * (1.0 - opacity)
            else:
                comp_value = comp_value_fg + torch.ones_like(bg_color) * (1.0 - opacity)

            out_sub[f"comp_{k}"] = comp_value.view(batch_size, height, width, -1)
        ...
        # disparity = torch.lerp(torch.zeros_like(disparity), disparity, (out["opacity"] > 0.5).float())

        out["disparity"] = disparity
        if "c2w" in kwargs:
            c2w = kwargs["c2w"]
        else:
            c2w = torch.eye(4).unsqueeze(0).repeat(batch_size, 1, 1).to(weights.device)
        ...
        if self.training:
            ...
            if "normal" in geo_out:
                if self.cfg.return_comp_normal:
                    ...
                    comp_normal_cam = self.convert_normal_to_cam_space(comp_normal, c2w)
                    ...
                    bg_normal_black = torch.zeros_like(comp_normal_cam)
                    comp_normal_cam_black_vis = (
                        comp_normal_cam + 1.0
                    ) / 2.0 * opacity + (1 - opacity) * bg_normal_black
                    ...
                    out.update(
                        {
                            ...
                            "comp_normal_cam_black_vis": comp_normal_cam_black_vis.view(
                                batch_size, height, width, 3
                            ),
                        }
                    )
            ...
            if "normal" in geo_out:
                ...
                bg_normal_white = torch.ones_like(comp_normal_cam)
                comp_normal_cam_white_vis = (comp_normal_cam + 1.0) / 2.0 * opacity + (
                    1 - opacity
                ) * bg_normal_white

                bg_normal_black = torch.zeros_like(comp_normal_cam)
                comp_normal_cam_black_vis = (comp_normal_cam + 1.0) / 2.0 * opacity + (
                    1 - opacity
                ) * bg_normal_black

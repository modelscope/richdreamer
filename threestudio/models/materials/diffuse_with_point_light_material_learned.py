...
@threestudio.register("diffuse-with-point-light-material-learned")
class DiffuseWithPointLightMaterial(BaseMaterial):
    ...
    def configure(self) -> None:
        ...
        self.mat_network = get_mlp(
            self.mat_encoding.n_output_dims, 3, self.cfg.mlp_network_config
        )
    ...
    def forward(
        ...
        **kwargs,
        ...
        shading = "diffuse"
        features = self.mat_network(self.mat_encoding(kwargs["points_scaled"]))
        albedo = get_activation(self.cfg.albedo_activation)(features)
        ...
        if shading == "albedo":
            out["rgb"] = albedo + textureless_color * 0
        elif shading == "textureless":
            out["rgb"] = albedo * 0 + textureless_color

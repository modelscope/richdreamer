...
from threestudio.utils.misc import (C, cleanup, get_device, get_rank,
                                    parse_version,)
...
class StableDiffusionDreamtimesGuidance(BaseObject):
    @dataclass
    class Config(BaseObject.Config):
        ...
        # dreamtimes
        dreamtimes: Optional[Any] = None  # [w1,w2,s1,s2]
        view_relative: bool = False
        min_values: float = 0.0
    ...
    def configure(self) -> None:
        ...
        if self.cfg.dreamtimes is not None:
            m1, m2, s1, s2 = self.cfg.dreamtimes
            m1 = int(m1 * self.cfg.max_step_percent)
            m2 = int(m2 * self.cfg.max_step_percent)
            s1 = s1 * self.cfg.max_step_percent
            s2 = s2 * self.cfg.max_step_percent

            total_iter = int(self.num_train_timesteps * self.cfg.max_step_percent)

            weights = torch.cat(
                (
                    torch.exp(
                        -((torch.arange(total_iter, m1, -1) - m1) ** 2) / (2 * s1**2)
                    ),
                    torch.ones(m1 - m2 + 1),
                    torch.exp(
        ...
        **kwargs,
        ...
        if self.cfg.dreamtimes is not None:
            ...
            if time_index == 0 or torch.abs(
                ...
                t = (
                    int(self.num_train_timesteps * self.cfg.max_step_percent)
                    - time_index
                )
            else:
                t = (
        ...
        if self.cfg.view_relative:
            relative_weights: Float[Tensor, "B"] = (180 - torch.abs(azimuth)) / 180.0
            relative_weights = relative_weights.clamp(min=self.cfg.min_values)
            loss_sds: Float[Tensor, "B 4 H W"] = 0.5 * F.mse_loss(
                latents, target, reduction="none"
            )
            relative_sds_loss = relative_weights[:, None, None, None] * loss_sds
            loss_sds = relative_sds_loss.sum() / batch_size
        else:

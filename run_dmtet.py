...
def main(args, extras):
    ...
    if args.iters is not None:
        cmd = f'bash scripts/dmtet/run_single.sh {args.gpus} "{args.text}" {args.output_dir} trainer.max_steps={args.iters}'
    else:
        cmd = f'bash scripts/dmtet/run_single.sh {args.gpus} "{args.text}" {args.output_dir} '
    print(f"cmd:{cmd}")
    os.system(cmd)
